...
export const EntryProvider = ({ children }) => {
  ...
  const submitForm = (e) => {
    ...
    if (isEditing === false) {
      ...
      setDailyPlan(dailyPlan.map(day => day !== editedDay ? day : {
        dayNumber,
        date,
        location,
        plan,
    ...
    if(dayNumber >= dailyPlan.length) {
      ...
      const year = dailyPlan[dailyPlan.length-1].date.toString().substring(0, 4)
      const month = dailyPlan[dailyPlan.length-1].date.toString().substring(5, 7)
      const day = dailyPlan[dailyPlan.length-1].date.toString().substring(8, 10)
